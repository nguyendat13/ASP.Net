    ...
    <div className="container-fluid mt-5" style={{ background: 'var(--background)', borderRadius: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', padding: '32px 0', minHeight: '80vh' }}>
      ...
      <div className="row justify-content-center align-items-center">
        <div className="col-12 col-md-5 mb-4 d-flex justify-content-center">
          <div style={{ background: 'var(--card)', borderRadius: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', padding: '16px' }}>
            <img
              src={`${API_BASE_URL}/api/Product/image/${product.avatar}`}
              alt={product.name}
              style={{ maxHeight: '350px', maxWidth: '100%', objectFit: 'contain', borderRadius: '12px', border: '2px solid var(--primary)', background: 'var(--card)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}
            />
          </div>
        </div>
        <div className="col-12 col-md-7">
          <h3 className="d-flex align-items-center" style={{ color: 'var(--foreground)', fontWeight: 'bold', fontSize: '2rem', textShadow: 'none' }}>
          ...
          <p className="d-flex align-items-center" style={{ fontSize: '1.3rem', color: 'var(--primary)', fontWeight: 'bold' }}>
            <FaMoneyBillWave style={{ color: 'var(--foreground)', marginRight: '8px' }} /> {product.price.toLocaleString()} đ
          </p>
          {product.discount > 0 && (
            <p className="d-flex align-items-center" style={{ color: 'var(--primary)', fontWeight: 'bold' }}>
              <FaPercent style={{ marginRight: '8px', color: 'var(--foreground)' }} /> Giảm giá: {product.discount}%
            </p>
          )}
          <p className="d-flex align-items-center" style={{ color: 'var(--foreground)', fontWeight: 'bold', textShadow: 'none' }}>
            <FaFolderOpen style={{ color: 'var(--primary)', marginRight: '8px' }} /> Danh mục: {product.categoryName}
          </p>
          <button className="btn btn-warning mt-3 px-4 py-2" style={{ color: 'var(--card)', fontWeight: 'bold', fontSize: '1.1rem', border: '2px solid var(--primary)', background: 'var(--primary)' }} onClick={handleAddToCart}>Thêm vào giỏ</button>
        </div>
      </div>
