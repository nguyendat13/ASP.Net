...
import { FaArrowLeft, FaUndoAlt, FaSearch } from "react-icons/fa";
...
const ReturnedOrdersList = () => {
  ...
  if (loading) {
  return <div className="text-center mt-5 text-light">Đang tải đơn hàng hoàn trả...</div>;
  }

  return (
    <div className="container mt-5">
      <h2 className="text-center mb-4 fw-bold" style={{ color: '#039be5', letterSpacing: 1 }}>
        <FaUndoAlt className="me-2" style={{ color: '#ff9800', fontSize: 28 }} />
        Đơn hàng hoàn trả
      </h2>
      ...
      {orders.length === 0 ? (
        <div className="alert alert-info text-center fw-bold" style={{ background: '#212121', color: '#039be5', border: 'none' }}>
        ...
        <div className="card shadow-lg" style={{ background: '#23272a', borderRadius: 16 }}>
          <div className="card-body">
            <table className="table table-bordered table-striped" style={{ background: '#23272a', color: '#fff', borderRadius: 12 }}>
              <thead className="table-dark" style={{ background: '#212121', color: '#039be5' }}>
                <tr>
                  <th>Mã đơn</th>
                  <th>Khách hàng</th>
                  <th>Ngày đặt</th>
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id} style={{ background: '#23272a', color: '#fff' }}>
                    ...
                    <td style={{ color: '#039be5', fontWeight: 600 }}>{order.statusName}</td>
